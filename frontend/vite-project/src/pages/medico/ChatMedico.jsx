...
export default function ChatMedico() {
  ...
  const fetchMessaggi = async () => {
    try {
      ...
      // Ricava nome paziente dal primo messaggio
  ...
  return (
    <div className="chat-container">
      ...
      <div className="chat-header">
        <button className="back-btn" onClick={() => navigate("/medico/conversazioni")}>
          ←
        </button>
      ...
      <div className="chat-messages">
        {messaggi.map((m) => (
          <div
            key={m.id}
            className={`chat-bubble ${m.mittente === "MEDICO" ? "medico" : "paziente"}`}
          >
            <p>{m.testo}</p>
            <span className="timestamp">
              {m.timestamp
                ? new Date(m.timestamp.replace(" ", "T")).toLocaleString("it-IT", {
                    day: "2-digit",
                    month: "2-digit",
                    year: "numeric",
                    hour: "2-digit",
